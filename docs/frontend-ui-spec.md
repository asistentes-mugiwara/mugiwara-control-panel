...
3. **Inicio es la entrada del producto.** `/` orienta y navega; no duplica métricas ni estados de módulos propietarios.
...
- Inicio existe como portada privada en `/`; no es landing pública ni dashboard operativo.
...
- Inicio como portada privada orientada a navegación, no como landing pública
- cards visuales hacia las superficies principales
- bloque de `Tripulación activa` como patrón de reconocimiento dentro de `/mugiwaras`
...
## 4.4 Inicio
### `HomeHero`
- identidad Mugiwara/Laya
- copy breve de producto privado/read-first
- sin métricas operativas duplicadas

### `HomeRouteCard`
- card de navegación a una superficie principal
- título, descripción, icono y CTA
- destino explícito a rutas internas

### `HomeRouteGrid`
- grid responsive de cards hacia Mugiwaras, Skills, Memory, Vault, Healthcheck, Repos Git y Uso
...
## 5.2 Inicio
### Objetivo
Entrada privada del producto en `/`: orienta y navega sin duplicar dashboards operativos.

### Estructura
```text
[HomeHero]
Título: Inicio
Subtítulo: punto de entrada privado al control panel Mugiwara/Hermes
Marca: Laya Mugiwara

[HomeRouteGrid]
┌────────────┐ ┌────────────┐ ┌────────────┐
│ Mugiwaras  │ │ Skills     │ │ Memory     │
└────────────┘ └────────────┘ └────────────┘
┌────────────┐ ┌────────────┐ ┌────────────┐
│ Vault      │ │ Healthcheck│ │ Repos Git  │
└────────────┘ └────────────┘ └────────────┘
┌────────────┐
│ Uso        │
└────────────┘
...
### Reglas visuales
- sin sidebar ni botón móvil de navegación lateral en `/`
- hero interno corto; no convertirlo en landing de marketing pública
- cards legibles y escaneables en desktop, tablet y móvil
- no mostrar métricas duplicadas ni concepto visible de Dashboard

### Estados
- Inicio debe ser estático y navegable; los estados operativos viven en sus módulos propietarios
- `/dashboard` redirige a `/` como alias temporal
...
## Inicio
- `ready`: hero y cards de navegación visibles
- `empty/error/stale`: no aplican a Inicio salvo fallo estructural del shell; los estados operativos pertenecen a sus módulos
...
- mantener Inicio como portada privada en `/`, no como landing pública ni dashboard operativo
...
## Shell
- [ ] Inicio en `/` visible sin sidebar ni botón móvil de navegación lateral
- [ ] rutas internas con sidebar estable y navegación a Inicio
- [ ] topbar consistente
...
**Inicio privado y navegable + identidad One Piece sutil + módulos operativos separados + fichas de Mugiwara con calavera y color propio.**
