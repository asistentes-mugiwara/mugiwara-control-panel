...
Decisión operativa: colocar una skill nueva bajo `skills-source/global`, `skills-source/agents/<mugiwara>` o el runtime root allowlisteado la incorpora al catálogo editable del panel si cumple el contrato `SKILL.md` y el slug de Mugiwara está allowlisteado. El backend mantiene un snapshot corto del catálogo y lo reconstruye en el siguiente request cuando vence su TTL server-side de 15 segundos; no hay watcher, timer en segundo plano ni refresh controlado por el cliente. Esto permite que altas/bajas de `SKILL.md` se reflejen sin reiniciar la API mientras se conserva resolución por `skill_id`, deny-by-default, rechazo de symlinks y ausencia de paths libres del cliente. Las skills globales se editan con dueño/actor visible `luffy`; las skills de agente usan su `owner_slug`; las runtime usan `runtime` salvo política posterior más granular. Si una skill debe quedar fuera de edición, debe retirarse de esta superficie hasta que exista política granular.
