...
| `hermes-gateways` | Franky | systemd user gateway summary | Phase 15.5a reads a fixed gateway status manifest and derives aggregated status from allowlisted per-Mugiwara entries only. |
| `gateway.<mugiwara-slug>` | Franky | allowlisted gateway status summary | Phase 15.5a reads one allowlisted process summary per Mugiwara slug from the fixed manifest. It does not expose PIDs, command lines, unit files, journal output, environment values, runtime paths or restart logs. |
