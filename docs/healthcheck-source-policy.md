...
| `cronjobs` | Franky | shared manifest registry, not Zoro profile-local `cronjob list` | Phase 15.6a reads a fixed status manifest and exposes only aggregate status/freshness derived from safe job fields. It must represent global scheduled jobs safely, not one profile's local runtime view. |
