...
- exponer `GET /api/v1/usage/five-hour-window-days` como read model simplificado para seleccionar un día de los últimos siete días naturales `Europe/Madrid`; las ventanas que cruzan medianoche se asignan al día local con mayor solape
...
- serializar actividad Hermes solo como agregados por perfil/rango: sesiones, mensajes, tool calls, tokens agregados semanales/totales, primera/última actividad, perfil dominante e índice bajo/medio/alto; sin prompts, conversaciones, tool payloads, tokens por sesión/conversación, IDs, targets, secretos, headers, cookies ni logs
...
Phase 18.4 automates `backup-health-status` with `mugiwara-backup-health-status.timer` and `scripts/install-backup-health-status-user-timer.sh`. The runner runs `npm run write:backup-health-status` from the fixed repo root, uses `TimeoutStartSec=360s`, and preserves a 60-second margin above the producer's 300-second checksum-validation timeout. It does not pass `--output` or `--backups-dir`, does not run backups, and keeps the backend contract unchanged: Healthcheck still consumes only the fixed sane manifest and never runs backup jobs, shell, systemd or filesystem discovery.
